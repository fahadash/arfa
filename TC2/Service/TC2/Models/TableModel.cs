...
namespace TC2.Models
{
    public class TableModel
    {
        ...
        public static object SubmitUserCard(Guid loginToken, int tableId, string card)
        {
            ...
            using (TC2DataContext ctx = TCDataContext.CreateContext())
            {
            ...
            // Submit the card
            game.SubmitPlayerCard(playerNumber, card);
        ...
        public static object ChooseTrump(Guid loginToken, int tableId, string trumpSuit)
        {
            ...
            try
            {
                using (TCDataContext tx = TCDataContext.CreateContext())
                {
        ...
        public static object GetTableUpdate(Guid loginToken, int tableId)
        {
            ...
            try
            {
                using (TCDataContext tx = TCDataContext.CreateContext())
                {
                    ...
                    {
                        var currentTableState = table.TableStates.SingleOrDefault(s => s.UserId.Equals(tableUser.UserId));
                        ...
                        if (isYou)
                        {
                            var query = table.TableUsers.Where(u => u.UserId.Equals(user.UserId)).FirstOrDefault().TableUserCards.Select(
                                 c => new
                                         {
                                             cardid = c.CardId,
                            ...
                            yourCards = query.ToArray();
                        }

                        users[userNumber] = new
                        {
                            userinfo = new
                                {
                            ...
                            gamescore = table.GameScores.Single(s => s.UserId.Equals(tableUser.UserId)).Score,
                            currentcard = currentCard != null? new
                              {
